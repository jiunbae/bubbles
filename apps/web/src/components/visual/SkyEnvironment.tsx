...
function RooftopEnvironment({ time, cameraMode }: { time: TimeColors; cameraMode?: boolean }) {
  ...
  return (
    <>
      {!cameraMode && <color attach="background" args={[bg]} />}
      <ambientLight color={cameraMode ? '#ffffff' : ambientColor} intensity={cameraMode ? 1.5 : 1.2 * time.ambientIntensity} />
      <directionalLight
        color={cameraMode ? '#ffffff' : new THREE.Color('#ffa54f').lerp(time.sunColor, 0.3)}
        intensity={cameraMode ? 2.0 : 3.0 * time.sunIntensity}
...
function ParkEnvironment({ time, cameraMode }: { time: TimeColors; cameraMode?: boolean }) {
  ...
  return (
    <>
      {!cameraMode && <color attach="background" args={[bg]} />}
      <ambientLight color={cameraMode ? '#ffffff' : ambientColor} intensity={cameraMode ? 1.5 : 1.0 * time.ambientIntensity} />
      <directionalLight
        color={cameraMode ? '#ffffff' : new THREE.Color('#eeeedd').lerp(time.sunColor, 0.3)}
        intensity={cameraMode ? 2.0 : 2.0 * time.sunIntensity}
        position={time.sunPosition}
      />
      {!cameraMode && (
        <>
...
function AlleyEnvironment({ time, cameraMode }: { time: TimeColors; cameraMode?: boolean }) {
  ...
  return (
    <>
      ...
      <ambientLight color={cameraMode ? '#ffffff' : ambientColor} intensity={cameraMode ? 1.5 : 1.0 * time.ambientIntensity} />
      <directionalLight
        color={cameraMode ? '#ffffff' : new THREE.Color('#889aaa').lerp(time.sunColor, 0.25)}
        intensity={cameraMode ? 2.0 : 1.0 * time.sunIntensity}
        position={time.sunPosition}
      />
      {!cameraMode && (
        <>
...
function ClearBackground({ active }: { active: boolean }) {
...
export function SkyEnvironment({ theme = 'rooftop', cameraMode }: SkyEnvironmentProps) {
  ...
  return (
    <>
      {cameraMode && <ClearBackground active />}
      {theme === 'park' ? (
        <ParkEnvironment time={time} cameraMode={cameraMode} />
      ) : theme === 'alley' ? (
        <AlleyEnvironment time={time} cameraMode={cameraMode} />
      ) : (
        <RooftopEnvironment time={time} cameraMode={cameraMode} />
      )}
    </>
  );
}
