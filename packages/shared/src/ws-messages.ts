...
export type ClientMessage =
  | { type: 'blow'; data: { bubbleId?: string; size: 'S' | 'M' | 'L'; color: string; pattern: 'plain' | 'spiral' | 'dots' | 'star'; x?: number; y?: number; z?: number; seed?: number; expiresAt?: number } }
...
export type ServerMessage =
  | { type: 'room_state'; ts: number; data: { placeId: string; placeName: string; mySessionId: string; users: UserInfo[]; bubbles: BubbleInfo[] } }
